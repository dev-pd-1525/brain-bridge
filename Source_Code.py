...
def power_off():
    print("BRAIN BRIDGE: Shutting down. Goodbye!")

def security():

    print("=="*35)

    print("B R A I N - B R I D G E")
    print("BRAIN BRIDGE: EDUCATIONAL HELP SYSTEM v1.1")
    print("Title line made by PRATEEKL DAS (Registration ID: 25BCE10599)")
    print("-----------------------------------")
...
def teacher_login():
    ...
    if login_ok:
        ...
        print("Loggin failed. Please check credentials.")
        security()
...
def student_login():
    ...
    if s_id in STUDENT_RECORDS:
        ...
        # Changed f-string to comma concatenation
        print("Student ID", s_id, "not found. Registering new student...")
...
def teacher_window():
    ...
    print("\n"+"***"*10)
    # Changed f-string to comma concatenation
    print("Welcome Teacher,", current_user['ID_num'] + "!")
...
def enter_student_score():
    ...
    if s_id not in STUDENT_RECORDS:
        # Changed f-string to comma concatenation
        print("Student ID", s_id, "not found.")
        teacher_window()
        return
    ...
    # Changed f-string to comma concatenation
    print("Score", score, "recordd for ID", s_id + ".") 
    teacher_window()
...
def view_class_stats():
    ...
    if not scores_list:
        # Changed f-string to comma concatenation
        print("No scores found for course '", course_name, "'.")
        teacher_window()
        return
    ...
    # Changed f-string to comma concatenation and string formatting
    print("CLASS average:", "{:.2f}%".format(class_average), "| GRADE:", class_letter)
    teacher_window()
...
def advanced_class_analysis():
    ...
    MATH_ENROLLED_SET = {'25BCE10599', '25BCE0001', '25BCE0002', 'Ram', 'Shyam'}
    CS_ENROLLED_SET = {'25BCE10599', '25BCE0003', '25BCE0004', 'Sita', 'Ram'} 
    ...
    # Changed f-string to comma concatenation
    print("Total Unique Students in Math/CS Combined:", Set_Length)

    only_in_math = MATH_ENROLLED_SET.difference(CS_ENROLLED_SET)
    # Changed f-string to comma concatenation
    print("Students ONLY in Math:", only_in_math)

    scores_for_python = np.array([s_obj.course_grades.get('Python Essentials', {}).get('finalScore', 0)
                                 for s_obj in STUDENT_RECORDS.values()])

    if scores_for_python.size > 0 and accumulator(scores_for_python) > 0:
        std_dev = np.std(scores_for_python)
        # Changed f-string to string formatting
        print("\npython Scores Standard Deviation:", "{:.2f}".format(std_dev))
    else:
        print("\nNOTE: Insufficient data for Python Scores Standard Deviation.")

    top_students = ['Ram', 'Shyam', 'Sita', 'Vipul']
...
def student_window():
    ...
    print("\n"+"***"*10)
    # Changed f-string to comma concatenation
    print("Hello,", student_obj.user_name + "!") 
    print("Available Commands:")
...
def view_my_grades(student_obj):
    ...
    for course, details in student_obj.course_grades.items():
        ...
        # Changed f-string to string formatting and comma concatenation
        print("Course:", course, "| Score:", "{:.2f}".format(score), "| Credits:", "{:.1f}".format(credits), "| Grade:", letter_grade)
    student_window()
...
def calculate_my_gpa(student_obj):
    ...
    print("\n--- Your GPA Summary ---")
    # Changed f-string to string formatting
    print("YOUR CUMULATIVE GPA IS:", "{:.2f}".format(gpa))
...
if __name__ == "__main__":
    STUDENT_RECORDS['25BCE10599'] = Student('25BCE10599', 'PRATEEKL DAS')
    STUDENT_RECORDS['25BCE10599'].course_grades['Python Essentials'] = {'credits': 4.0, 'finalScore': 92.5}
    
    STUDENT_RECORDS['25BCE0001'] = Student('25BCE0001', 'Ram Kumar')
    STUDENT_RECORDS['25BCE0001'].course_grades['Python Essentials'] = {'credits': 4.0, 'finalScore': 85.0}

    STUDENT_RECORDS['25BCE0002'] = Student('25BCE0002', 'Shyam Lal')
    STUDENT_RECORDS['25BCE0002'].course_grades['Database Systems'] = {'credits': 3.0, 'finalScore': 78.0}
